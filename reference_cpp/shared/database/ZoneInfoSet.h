...
public:
	...
	virtual tstring GetTableName() { return _T("ZONE_INFO2369"); }
#elif GAME_SOURCE_VERSION  == 1098
	virtual tstring GetTableName() { return _T("ZONE_INFO1098"); }
#elif GAME_SOURCE_VERSION  == 1534
	virtual tstring GetTableName() { return _T("ZONE_INFO1534"); }
#endif
	virtual tstring GetColumns()
	{
		return _T("ServerNo, ZoneNo, strZoneSmdName, strZoneName,"
			...
			"Teleport,Gate,[Escape],CallingFriend,TeleportFriend,Blink,PetSpawn, "     // MenissiahList Düzenleme
			"ExpLost,GiveLoyalty,GuardSummon,MenissiahList,isMilitaryZone, isMiningZone, BlinkZone,autoloot, goldlose, InitX, InitZ, InitY");
	}
	...
	virtual bool Fetch()
	{
		...
		_dbCommand->FetchByte(i++, pData->m_bMenissiahList);
		_dbCommand->FetchByte(i++, pData->m_bMilitaryZone);
		_dbCommand->FetchByte(i++, pData->m_bMiningZone);
		_dbCommand->FetchByte(i++, pData->m_bBlinkZone);
		_dbCommand->FetchByte(i++, pData->m_bAutoLoot);
		_dbCommand->FetchByte(i++, pData->m_bGoldLose);
