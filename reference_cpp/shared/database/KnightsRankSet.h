...
public:
	...
	virtual bool Fetch()
	{
		...
		if (pKnights == nullptr
			|| (pKnights->m_byNation != (uint8)Nation::KARUS && pKnights->m_byNation != (uint8)Nation::ELMORAD)
		...
		pKnights->m_byRanking = pData->nRank;

		if (pKnights->m_byNation == (uint8)Nation::KARUS)
		{
			if (nKarusCount == 5)
				return true;

			CUser *pUser = g_pMain->GetUserPtr(pKnights->m_strChief, NameType::TYPE_CHARACTER);
			if (pUser == nullptr || !pUser->GetMap() || !pUser->GetMap()->isWarZone())
				return true;

			if (pUser->GetClanID() == pData->sClanID)
			{
				_COMMANDER *pCommander = new _COMMANDER();
				pCommander->strName = pUser->GetName();
				pCommander->bNation = pUser->GetNation();

				g_pMain->m_CommanderArrayLock.lock();
				if (!g_pMain->m_CommanderArray.insert(std::make_pair(pCommander->strName, pCommander)).second)
					delete pCommander;
		...
		else if (pKnights->m_byNation == (uint8)Nation::ELMORAD)
		{
			if (nElmoCount == 5)
				return true;
