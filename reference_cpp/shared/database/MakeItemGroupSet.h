...
public:
	...
	virtual bool Fetch()
	{
		...
		_dbCommand->FetchUInt32(field++, pData->iItemGroupNum);
		for (int i = 0; i < 200; i++) {
			uint32 iItem = 0;
			_dbCommand->FetchUInt32(field++, iItem);
			if (iItem == 0)
				continue;
...
class CMakeItemGroupRandomSet : public OdbcRecordset
{
public:
	CMakeItemGroupRandomSet(OdbcConnection* dbConnection, MakeItemGroupRandomArray* pMap)
		: OdbcRecordset(dbConnection), m_pMap(pMap) {}

	virtual tstring GetTableName() { return _T("MAKE_ITEM_GROUP_RANDOM"); }
	virtual tstring GetColumns() { return _T("nIndex, ItemID, GroupNo"); }

	virtual bool Fetch()
	{
		int field = 1;
		_MAKE_ITEM_GROUP_RANDOM* pData = new _MAKE_ITEM_GROUP_RANDOM;
		_dbCommand->FetchUInt32(field++, pData->nIndex);
		_dbCommand->FetchUInt32(field++, pData->ItemID);
		_dbCommand->FetchUInt32(field++, pData->GroupNo);
	...
	MakeItemGroupRandomArray* m_pMap;
};