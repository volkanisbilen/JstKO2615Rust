...
void KOSocket::OnRead()
{
	...
	for (;;)
	{
		...
		/*printf("Send:0x%02X ", pkt.GetOpcode());
...
bool KOSocket::Send(Packet * pkt)
{
	...
	//{
	//	uint16 OldGAS = GetWriteBuffer().GetAllocatedSize();
	//	//GetWriteBuffer().Allocate(GetWriteBuffer().GetAllocatedSize() + len + 12);
	//	printf("%s[%d] : WSASocket Error\n", GetRemoteIP().c_str(), GetSocketID());
	//	BurstEnd();
	//	Disconnect();
	//	return false;
	//}
