...
void SMDFile::LoadRegeneEvent(FILE* fp)
{
	...
	for (int i = 0; i < iEventObjectCount; i++)
	{
		...
		if (pEvent->sRegenePoint < 0 || !m_ObjectRegeneArray.PutData(pEvent->sRegenePoint, pEvent))
			delete pEvent;
	}
}
...
void SMDFile::LoadWarpList(FILE* fp)
{
	...
	for (int i = 0; i < WarpCount; i++) {
		...
		if (pWarp->sWarpID == 0 || !m_WarpArray.PutData(pWarp->sWarpID, pWarp))
			delete pWarp;
	}
}
...
void SMDFile::GetWarpList(int warpGroup, std::set<_WARP_INFO*>& warpEntries)
{
	foreach_stlmap(itr, m_WarpArray) {
		...
		if (pWarp == nullptr || (pWarp->sWarpID / 10) != warpGroup) continue;
...
{
	if (x < 0 || x >= m_nMapSize || z < 0 || z >= m_nMapSize) return -1;
	return m_ppnEvent[x * m_nMapSize + z];
}

SMDFile::~SMDFile()
{
	if (m_ppnEvent != nullptr) { delete[] m_ppnEvent; m_ppnEvent = nullptr; }
	if (m_fHeight != nullptr) { delete[] m_fHeight; m_fHeight = nullptr; }
	delete m_N3ShapeMgr;
}