...
#define MAX_BOT			5000 // JstKO Botlar MaxBot Eklendi 31.12.2024
...
enum class dbreqtype { Database, Logger, COUNT };
...
enum PartyType //Partyde HP Degeri Yazi Olarak Gosterme
{
	Send_All = 0,
	Send = 1
};
...
struct	_ITEM_DATA
{
	...
	INLINE bool isMerchantItem() { return MerchItem; }
};
...
struct _BOT_DATA
{
	...
	INLINE bool JobGroupCheck(short jobgroupid)
	{
		if (jobgroupid > 100)
		...
		switch (jobgroupid)
		{
		case 1: return (subClass == ClassWarrior || subClass == ClassWarriorNovice || subClass == ClassWarriorMaster);
		case 13: return (subClass == ClassPortuKurian || subClass == ClassPortuKurianNovice || subClass == ClassPortuKurianMaster);
		case 2: return (subClass == ClassRogue || subClass == ClassRogueNovice || subClass == ClassRogueMaster);
		case 3: return (subClass == ClassMage || subClass == ClassMageNovice || subClass == ClassMageMaster);
		case 4: return (subClass == ClassPriest || subClass == ClassPriestNovice || subClass == ClassPriestMaster);
		}
...
struct	_CHARACTER_SEAL_ITEM
{
	...
	void Initialize()
	{
		...
		for (int i = 0; i < 10; i++) m_bstrSkill[i] = 0;
		for (int i = 0; i < 5; i++) m_bStats[i] = 0;
		for (int i = 0; i < 5; i++) m_bRebStats[i] = 0;
	}
...
struct PET_DATA
{
	...
	_ITEM_DATA	sItem[PET_INVENTORY_TOTAL];
...
struct AKARA_LIST
{
	...
	AKARA_LIST(uint32 ItemID[], uint32 RealItemID[], uint32 Bett[], uint32 BetMaxx[], std::string Ownerr[])
	{
		for (int i = 0; i < 3; i++)
		{
			ItemNum[i] = ItemID[i];
			RealItemNum[i] = RealItemID[i];
			Bet[i] = Bett[i];
			BetMax[i] = BetMaxx[i];
			Owner[i] = Ownerr[i];
		}
...
//struct _BOT_MERCHANT_ITEM
//{
//	uint32 sIndex;
//	uint8 BotMerchantType;
//	std::string BotItemNum;
//	std::string BotItemCount;
//	std::string BotItemPrice;
//	std::string BotMerchantMessage;
//	_MERCH_DATA	m_MerchantItems[MAX_MERCH_ITEMS];
//
//	float	fX, fY, fZ;
//	uint32	iMinutes;
//	uint8	bZone;
...
enum AuthorityTypes
{
	...
	AUTHORITY_PLAYER				= 1,
	AUTHORITY_GM_USER				= 2, //aninda gm için
};
...
#define STAT_MAX 255
#define QUEST_ARRAY_SIZE	3500 // That's a limit of 500 quests (7 bytes per quest)
#define QUEST_LIMIT			(QUEST_ARRAY_SIZE / 7)

#define QUEST_DAILYARRAY_SIZE 2400
#define QUEST_DAILYLIMIT	(QUEST_DAILYARRAY_SIZE / 8)
...
INLINE std::string regex_quotes(std::string a) { return std::regex_replace(a, std::regex("\\'"), "''"); }
...
#define RLOCK(lock) lock.b_lock.r_lock();
#define RULOCK(lock) lock.b_lock.r_unlock();