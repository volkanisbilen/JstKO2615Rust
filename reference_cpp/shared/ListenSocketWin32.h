...
public:
	...
	bool runnable()
	{
		...
		while (m_opened && m_threadRunning)
		{
			...
			char* ip = inet_ntoa(m_tempAddress.sin_addr);
			std::string strIP(ip);
			 //Xtreme eski port 15001 yeni 15572
			if (m_socketMgr->m_threadPort == 15572 || m_socketMgr->m_threadPort == 15100)
			{
				bool check = SocketOps::CheckedConnect(aSocket, strIP);
				if (!check)
				{
					SocketOps::CloseSocket(aSocket, strIP);
					continue;
				}
			}

			// Attempt to assign the socket to an available session
			Socket* socket = m_socketMgr->AssignSocket(aSocket, m_tempAddress);		// Idle to active sessions
