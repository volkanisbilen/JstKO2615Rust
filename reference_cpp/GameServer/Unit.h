...
enum class AttackType { AttackTypeNone, AttackTypePhysical, AttackTypeMagic };
enum class UnitType { UnitPlayer, UnitNPC, UnitBot };
...
public:
	...
	INLINE bool isPlayer() { return GetUnitType() == UnitType::UnitPlayer; }
	INLINE bool isNPC() { return GetUnitType() == UnitType::UnitNPC; }
	INLINE bool isBot() { return GetUnitType() == UnitType::UnitBot; }

	INLINE KOMap * GetMap() { return m_pMap; }

	virtual uint16 GetID() = 0;
	INLINE uint8 GetZoneID() { return m_bZone; }
	INLINE bool isInMoradon(uint8 qzoneid = 0) {
		uint8 zoneid = GetZoneID();
		if (qzoneid) zoneid = qzoneid;
		return zoneid == ZONE_MORADON || zoneid == ZONE_MORADON2 || zoneid == ZONE_MORADON3 || zoneid == ZONE_MORADON4 || zoneid == ZONE_MORADON5;
	}

	INLINE bool isInLufersonCastle(uint8 qzoneid = 0) {
		uint8 zoneid = GetZoneID();
		if (qzoneid) zoneid = qzoneid;
		return zoneid == ZONE_KARUS || zoneid == ZONE_KARUS2 || zoneid == ZONE_KARUS3;
	}

	INLINE bool isInKarusEslant(uint8 qzoneid = 0) {
		uint8 zoneid = GetZoneID();
		if (qzoneid) zoneid = qzoneid;
		return zoneid == ZONE_KARUS_ESLANT || zoneid == ZONE_KARUS_ESLANT2 || zoneid == ZONE_KARUS_ESLANT3;
	}

	INLINE bool isInElmoradCastle(uint8 qzoneid = 0) {
		uint8 zoneid = GetZoneID();
		if (qzoneid) zoneid = qzoneid;
		return zoneid == ZONE_ELMORAD || zoneid == ZONE_ELMORAD2 || zoneid == ZONE_ELMORAD3;
	}
	INLINE bool isInElmoradEslant(uint8 qzoneid = 0) {
		uint8 zoneid = GetZoneID();
		if (qzoneid) zoneid = qzoneid;
	...
	INLINE bool isPartnerPartyZone() {
		return GetZoneID() == ZONE_MORADON
			...
			|| isInElmoradEslant() || isInKarusEslant();
	}
	...
	INLINE bool isInMonsterStoneZone(uint8 nZoneID = 0)
	{
		if (nZoneID == 0)
			nZoneID = GetZoneID();
	...
	INLINE bool isInSpecialEventZone(uint8 nZoneID = 0) {
		if (nZoneID == 0) nZoneID = GetZoneID();
		return (nZoneID >= ZONE_SPBATTLE1 && nZoneID <= ZONE_SPBATTLE12);
	}
	...
	INLINE bool isTransformed() { return m_transformationType != TransformationType::TransformationNone; }
	INLINE bool isNPCTransformation() { return m_transformationType == TransformationType::TransformationNPC; }
	INLINE bool isMonsterTransformation() { return m_transformationType == TransformationType::TransformationMonster; }
	INLINE bool isSiegeTransformation() { return m_transformationType == TransformationType::TransformationSiege; }
	...
	float Get3Distance(float fx, float fz, float fy);
	...
	bool isInRange3D(float fx, float fz, float fy, float fSquaredRange);
	...
	static float Get3Distance(float fStartX, float fStartZ, float fStartY, float fEndX, float fEndZ, float fEndY);
	...
	bool isInAttackRange(Unit * pTarget, _MAGIC_TABLE pSkill = _MAGIC_TABLE());
	virtual short GetDamage(Unit *pTarget, _MAGIC_TABLE pSkill = _MAGIC_TABLE(), bool bPreviewOnly = false) = 0;
	...
	virtual void HpChangeMagic(int amount, Unit *pAttacker = nullptr, AttributeType attributeType = AttributeType::AttributeNone) { HpChange(amount, pAttacker); }
	...
	struct MagicType3
	{
		...
		uint32_t	m_skill;// target Scroll Bar Eklemesi 03.02.2024 end
		MagicType3() { Reset(); }
		...
		INLINE void Reset()
		{
			...
			m_bTickLimit = 0;
			m_sSourceID = -1;
			m_byAttribute = (uint8)AttributeType::AttributeNone;
			m_skill = 0;// target Scroll Bar Eklemesi 03.02.2024 end
		}
	};
...
enum class TransformationType
{
	...
	TransformationSkillUse m_transformSkillUseid;
	...
	std::recursive_mutex  m_buffLock,m_buff9lock;
