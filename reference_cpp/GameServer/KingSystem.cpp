...
void CKingSystem::LoadRecommendList()
{
	...
	foreach_stlmap(itr, g_pMain->m_KnightsRatingArray[m_byNation - 1])
	{
		...
		CUser * pUser = g_pMain->GetUserPtr(pKnights->m_strChief, NameType::TYPE_CHARACTER);
		if (pUser != nullptr)
		{
			Packet result;
			std::string notice;
...
void CKingSystem::InsertNominee(std::string & strNominator, std::string & strNominee, uint16 sNomineeKnights)
{
	...
	_KING_ELECTION_LIST * pEntry = new _KING_ELECTION_LIST;  //  açtı problem olursa burası kapatılacak 31.05.2021
	memcpy(pEntry, senatorItr->second, sizeof(_KING_ELECTION_LIST));  //  açtı problem olursa burası kapatılacak 31.05.2021
	//_KING_ELECTION_LIST * pEntry = new _KING_ELECTION_LIST;
	pEntry->nVotes = 0;
...
void CKingSystem::ImpeachmentElectionUiOpen(CUser * pUser, Packet & pkt)
{
	...
	result << uint8(KING_IMPEACHMENT_ELECTION_UI_OPEN) << int16(m_byImType != 3 ? -1 : 1);
	pUser->Send(&result);
}
#pragma endregion
...
void CKingSystem::KingTaxSystem(CUser * pUser, Packet & pkt)
{
	...
	case 7:
	{
		...
		{
			pUser->GiveItem("King Item",KING_SCEPTER);
			pUser->GiveItem("King Item",KING_SCEPTER);
			result << int16(1);
		}
		pUser->Send(&result);
...
void CKingSystem::KingSpecialEvent(CUser * pUser, Packet & pkt)
{
	...
	case KING_EVENT_NOAH: // Noah event
	{
		...
		m_KingSystemlock.unlock();
		g_pMain->SendFormattedResource(m_byNation == (uint8)Nation::KARUS ? IDS_KING_KARUS_NOAH_BONUS_EVENT : IDS_KING_ELMO_NOAH_BONUS_EVENT, m_byNation, false, bAmount);
	...
	case KING_EVENT_EXP: // EXP event
	{
		...
		g_pMain->SendFormattedResource(m_byNation == (uint8)Nation::KARUS ? IDS_KING_KARUS_EXP_BONUS_EVENT : IDS_KING_ELMO_EXP_BONUS_EVENT,
	...
	case KING_EVENT_PRIZE:
	{
		...
		m_KingSystemlock.unlock();
		// (awarded %s %d coins)
		g_pMain->SendFormattedResource(m_byNation == (uint8)Nation::KARUS ? IDS_KING_KARUS_PRIZE_EVENT_MESSAGE : IDS_KING_ELMO_PRIZE_EVENT_MESSAGE,
	...
	case KING_EVENT_NOTICE:
	{
		...
		pUser->ChatInsertLog(100, "KING", strMessage, nullptr);
		result.SByte();
