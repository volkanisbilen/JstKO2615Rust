...
#pragma region CUser::UpdateAchievePlayTime()
/*
* @brief	Update the user summary Play information.
*
* @param	pkt update 
*/
void CUser::UpdateAchievePlayTime()
{
	if (m_bAchieveLoginTime <= 0) return;

	uint32 UpdatePlayTime = 0;
	if ((uint32)UNIXTIME > m_bAchieveLoginTime) 
		UpdatePlayTime = uint32(UNIXTIME - m_bAchieveLoginTime);

	m_AchieveInfo.PlayTime += UpdatePlayTime;
...
void CUser::HandleUserAchieveSummary(Packet &pkt)
{
	...
	result << uint32(m_AchieveInfo.PlayTime / 60)
...
*/
void CUser::HandleUserAchieveGiftRequest(Packet & pkt)//x1x0x0
{
	uint16 sIndex = 0;
	pkt >> sIndex;

	if (pCindWar.isEventUser() && g_pMain->isCindirellaZone(GetZoneID())) return;

	Packet result(WIZ_USER_ACHIEVE, uint8(UserAchieveOpcodes::AchieveGiftRequest));
	_ACHIEVE_MAIN* pMain = g_pMain->m_AchieveMainArray.GetData(sIndex);
	if (pMain == nullptr) {
		result << sIndex << uint16(0);
		Send(&result);
		return;
	}

	_ITEM_TABLE item = g_pMain->GetItemPtr(pMain->ItemNum);
	if (item.isnull()) {
		result << sIndex << uint16(0);
		Send(&result);
		return;
	...
	if (!GiveItem("Achieve Gift", item.m_iNum, (uint16)pMain->iCount)) {
		result << sIndex << uint16(0);
		Send(&result);
		return;
	}
...
* bStatus = 1: Unfinished 04: Finished 05: Complete(items taken)
*/
void CUser::HandleUserAchieveUserDetail(Packet & pkt)//x1x0x0
{
...
void CUser::AchieveMonsterCountAdd(uint16 sNpcID)
{
	...
	foreach_stlmap(itr, m_AchieveMap)
	{
		...
		switch ((UserAchieveMainTypes)pAchieveMain->Type)
		{
		case UserAchieveMainTypes::AchieveWar:
		{
			_ACHIEVE_WAR *pAchieveWar = g_pMain->m_AchieveWarArray.GetData(pAchieveMain->sIndex);
			if (pAchieveWar == nullptr
				|| pAchieveWar->Type != (uint8)UserAchieveWarTypes::AchieveDefeatMonsterRonarkL)
				continue;

			AchieveWarCheck(pAchieveData, pAchieveWar, pAchieveMain, sNpcID, nullptr);
		}
		break;
		case UserAchieveMainTypes::AchieveMonster:
		{
...
void CUser::AchieveWarCountAdd(UserAchieveWarTypes type, uint16 sNpcID /*= 0*/, CUser *pTarget /*= nullptr*/)
{
	foreach_stlmap(itr, m_AchieveMap)
	{
		...
		if (pAchieveMain == nullptr
			|| (pAchieveMain->ZoneID > 0 && pAchieveMain->ZoneID != GetZoneID())
			|| pAchieveMain->Type != (uint8)UserAchieveMainTypes::AchieveWar)
			continue;
...
void CUser::AchieveNormalCountAdd(UserAchieveNormalTypes type, uint16 sNpcID /*= 0*/, CUser *pTarget /*= nullptr*/)
{
	foreach_stlmap(itr, m_AchieveMap)
	{
		...
		if (pAchieveMain == nullptr
			|| (pAchieveMain->ZoneID > 0 && pAchieveMain->ZoneID != GetZoneID())
			|| pAchieveMain->Type != (uint8)UserAchieveMainTypes::AchieveNormal)
			continue;
		...
		case UserAchieveMainTypes::AchieveNormal:
		{
			_ACHIEVE_NORMAL *pAchieveNormal = g_pMain->m_AchieveNormalArray.GetData(pAchieveMain->sIndex);
			if (pAchieveNormal == nullptr
				|| pAchieveNormal->Type != (uint8)type)
				continue;
...
void CUser::AchieveComCountAdd(UserAchieveComTypes type, uint16 sNpcID/* = 0*/, CUser *pTarget/* = nullptr*/)
{
	...
	foreach_stlmap(itr, m_AchieveMap)
	{
		...
		if (pAchieveMain->ZoneID > 0 && pAchieveMain->ZoneID != GetZoneID() || pAchieveMain->Type != (uint8)UserAchieveMainTypes::AchieveCom)
			continue;

		// if the Achievement is completed or finished, why would we check for 'em?
		if (pAchieveData->bStatus == UserAchieveStatus::AchieveCompleted || pAchieveData->bStatus == UserAchieveStatus::AchieveFinished)
			continue;

		switch ((UserAchieveMainTypes)pAchieveMain->Type)
		{
		case UserAchieveMainTypes::AchieveCom:
		{
			_ACHIEVE_COM *pAchieveCom = g_pMain->m_AchieveComArray.GetData(pAchieveMain->sIndex);
			if (pAchieveCom == nullptr
				|| pAchieveCom->Type != (uint8)type)
				continue;
...
*
*/
void CUser::AchieveWarCheck(_USER_ACHIEVE_INFO *pAchieveData, _ACHIEVE_WAR *pAchieveWar, _ACHIEVE_MAIN *pAchieveMain, uint16 sNpcID/* = 0*/, CUser *pTarget/* = nullptr*/)//x1x0x0
{
	bool bAchieveFinished = false;
	if (pAchieveWar == nullptr
		|| pAchieveData == nullptr
		|| pAchieveMain == nullptr)
		return;

	switch ((UserAchieveWarTypes)pAchieveWar->Type)
	{
	case UserAchieveWarTypes::AchieveDefeatEnemy:
	{
		if (pTarget == nullptr)
			return;

		if ((pAchieveMain->ZoneID > 0 && pTarget->GetZoneID() != pAchieveMain->ZoneID)
	...
	break;
	case UserAchieveWarTypes::AchieveTakeRevange:
		pAchieveData->iCount[0]++;
		if (pAchieveWar->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveWar->Count;
		...
		break;
	case UserAchieveWarTypes::AchieveWinWar:
		m_AchieveInfo.WarCount++;
		...
		break;
	case UserAchieveWarTypes::AchieveWinNationGuardBattle:
		pAchieveData->iCount[0]++;
		if (pAchieveWar->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveWar->Count;
		...
		break;
	case UserAchieveWarTypes::AchieveWinJuraid:
		pAchieveData->iCount[0]++;
		if (pAchieveWar->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveWar->Count;
		...
		break;
	case UserAchieveWarTypes::AchieveWinChaos1:
		pAchieveData->iCount[0]++;
		if (pAchieveWar->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveWar->Count;
		...
		break;
	case UserAchieveWarTypes::AchieveWinChaos2:
		pAchieveData->iCount[0]++;
		if (pAchieveWar->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveWar->Count;
		...
		break;
	case UserAchieveWarTypes::AchieveWinChaos3:
		pAchieveData->iCount[0]++;
		if (pAchieveWar->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveWar->Count;
		...
		break;
	case UserAchieveWarTypes::AchieveWinCSW:
		pAchieveData->iCount[0]++;
		if (pAchieveWar->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveWar->Count;
		...
		break;
	case UserAchieveWarTypes::AchieveKillCountChaos:
		pAchieveData->iCount[0]++;
		if (pAchieveWar->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveWar->Count;
		...
		break;
	case UserAchieveWarTypes::AchieveDefeatMonsterRonarkL:
		...
		break;
	case UserAchieveWarTypes::AchieveDrakiTowerFinished:
		pAchieveData->iCount[0]++;
		if (pAchieveWar->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveWar->Count;
...
void CUser::AchieveNormalCheck(_USER_ACHIEVE_INFO *pAchieveData, _ACHIEVE_NORMAL *pAchieveNormal, _ACHIEVE_MAIN *pAchieveMain, uint16 sNpcID /*= 0*/, CUser *pTarget /*= nullptr*/)
{
	...
	if (pAchieveData == nullptr || pAchieveNormal == nullptr || pAchieveMain == nullptr)
		return;

	switch ((UserAchieveNormalTypes)pAchieveNormal->Type)
	{
	case UserAchieveNormalTypes::AchieveBecomeKing:
	{
	...
	break;
	case UserAchieveNormalTypes::AchieveReachNP:
		...
		break;
	case UserAchieveNormalTypes::AchieveReachLevel:
		...
		break;
	case UserAchieveNormalTypes::AchieveBecomeSpecial:
		pAchieveData->iCount[0]++;
		if (pAchieveNormal->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveNormal->Count;
		...
		break;
	case UserAchieveNormalTypes::AchieveKnightContribution: // Achieve Knight Contribution Point of XXX
		...
		break;
	case UserAchieveNormalTypes::AchieveDefeatMons: // Do not get killed and defeat XXX monsters
		pAchieveData->iCount[0]++;
		if (pAchieveNormal->Count <= pAchieveData->iCount[0])
		{
			pAchieveData->iCount[0] = pAchieveNormal->Count;
		...
		break;
	case UserAchieveNormalTypes::AchieveBecomeKnightMember:
		if (GetClanID() > NO_CLAN)
		{
			...
			if (pAchieveNormal->sIndex == 419
				&& pKnights->m_byFlag == (uint8)ClanTypeFlag::ClanTypeTraining) // Becoming a member of Knight Trainees
			{
				pAchieveData->iCount[0]++;
				bAchieveFinished = true;
			}
				...
				&& pKnights->m_byFlag > (uint8)ClanTypeFlag::ClanTypeTraining) // Becoming a member of Knights
			{
				pAchieveData->iCount[0]++;
				bAchieveFinished = true;
			}
				...
				&& pKnights->GetID() == g_pMain->pSiegeWar.sMasterKnights) // Becoming a member of Castle Knights
			{
				pAchieveData->iCount[0]++;
				bAchieveFinished = true;
			}
...
{
	if (!pAchieveCom || !pAchieveData || !pAchieveMain)
		return false;
	...
	case UserAchieveComTypes::AchieveRequireAchieve:
	{
		...
		if (pAchieveInfoReq1 != nullptr && (pAchieveInfoReq1->bStatus == UserAchieveStatus::AchieveCompleted || pAchieveInfoReq1->bStatus == UserAchieveStatus::AchieveFinished))
		{
			if (pAchieveCom->Required2 > 0)
			{
				_USER_ACHIEVE_INFO* pAchieveInfoReq2 = m_AchieveMap.GetData(pAchieveCom->Required2);
				if (pAchieveInfoReq2 != nullptr && (pAchieveInfoReq2->bStatus == UserAchieveStatus::AchieveCompleted || pAchieveInfoReq2->bStatus == UserAchieveStatus::AchieveFinished))
				{
					pAchieveData->iCount[0]++;
					bAchieveFinished = true;
				}
...
{
	if (pAchieveData == nullptr || pAchieveMonster == nullptr || pAchieveMain == nullptr)
		return;
...
* @param	pkt	The packet.
*/
void CUser::HandleUserAchieveCoverTitle(Packet & pkt)//x1x0x0
{
	...
	if (pAchieveInfo == nullptr || pAchieveMain == nullptr)
	{
		result << uint8(UserAchieveOpcodes::AchieveCoverTitleReset) << uint16(sCoverID) << uint16(sSkillID) << uint8(1) << uint8(0);
		Send(&result);
		return;
	}

	if (pAchieveInfo->bStatus != UserAchieveStatus::AchieveFinished
		&& pAchieveInfo->bStatus != UserAchieveStatus::AchieveCompleted)
	{
		result << uint8(UserAchieveOpcodes::AchieveCoverTitleReset) << uint16(sCoverID) << uint16(sSkillID) << uint8(1) << uint8(0);
		Send(&result);
		return;
	...
	result << uint8(UserAchieveOpcodes::AchieveCoverTitle) << uint16(sCoverID) << uint16(sSkillID) << uint8(1) << uint8(0);
	Send(&result);
}
...
* @param	pkt	The packet.
*/
void CUser::HandleUserAchieveSkillTitle(Packet & pkt)//x1x0x0
{
	uint16 sCoverID, sSkillID;
	pkt >> sCoverID >> sSkillID;
	...
	if (pAchieveInfo == nullptr || pAchieveTitle == nullptr)
	{
		m_sSkillTitle = 0;
		result << uint8(UserAchieveOpcodes::AchieveSkillTitleReset) << uint16(sCoverID) << uint16(sSkillID) << uint8(1) << uint8(0);
		Send(&result);
		return;
	}

	if (pAchieveInfo->bStatus != UserAchieveStatus::AchieveFinished && pAchieveInfo->bStatus != UserAchieveStatus::AchieveCompleted)
	{
		m_sSkillTitle = 0;
		result << uint8(UserAchieveOpcodes::AchieveSkillTitleReset) << uint16(sCoverID) << uint16(sSkillID) << uint8(1) << uint8(0);
		Send(&result);
	...
	m_sSkillID = sCoverID; m_sSkillTitle = pAchieveMain->TitleID;
	result << uint8(AchieveSkillTitle) << uint16(sCoverID) << uint16(m_sSkillTitle) << uint8(1) << uint8(0);
	Send(&result);

	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_STR] = pAchieveTitle->Str;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_STA] = pAchieveTitle->Hp;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_DEX] = pAchieveTitle->Dex;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_INT] = pAchieveTitle->Int;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_CHA] = pAchieveTitle->Mp;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_ATTACK] = pAchieveTitle->Attack;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_DEFENCE] = pAchieveTitle->Defence;
	SetUserAbility(true);
}
...
void CUser::HandleUserAchieveSkillTitleReset(Packet & pkt)
{
	...
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_STR] = 0;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_STA] = 0;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_DEX] = 0;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_INT] = 0;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_CHA] = 0;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_ATTACK] = 0;
	m_sStatAchieveBonuses[(int16)UserAchieveStatTypes::ACHIEVE_STAT_DEFENCE] = 0;
