...
void CUser::FriendProcess(Packet & pkt)
{
	...
	case FRIEND_REMOVE:
		...
		break;
	default:
		printf("Friend Process unhandled opcode %d", opcode);
		TRACE("Friend Process unhandled opcode %d", opcode);
		break;
	}
}
...
void CUser::FriendModify(Packet & pkt, uint8 opcode)
{
	...
	if (strUserID.empty() || strUserID.size() > MAX_ID_SIZE
		|| (opcode == FRIEND_ADD && (pUser = g_pMain->GetUserPtr(strUserID, NameType::TYPE_CHARACTER)) == nullptr)) {
		CBot *pUser = g_pMain->GetBotPtr(strUserID, NameType::TYPE_CHARACTER);
		if (pUser == nullptr)
			return;
...
uint8 CUser::GetFriendStatus(std::string & charName, int16 & sid)
{
	CUser *pUser;
	if (charName.empty()
		|| (pUser = g_pMain->GetUserPtr(charName, NameType::TYPE_CHARACTER)) == nullptr)
	{
		CBot *pUser = g_pMain->GetBotPtr(charName, NameType::TYPE_CHARACTER);
		if (pUser == nullptr)
		{
			sid = -1;
