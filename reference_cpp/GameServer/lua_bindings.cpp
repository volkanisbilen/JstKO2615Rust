...
#define LUA_CLASS CUser
	...
	(
		...
		MAKE_LUA_METHOD(GetRebirthLevel)
		MAKE_LUA_METHOD(GetExp)
		...
		MAKE_LUA_METHOD(isBeginnerKurianPortu)
		...
		MAKE_LUA_METHOD(isNoviceKurianPortu)
		...
		MAKE_LUA_METHOD(isMasteredKurianPortu)
		...
		MAKE_LUA_METHOD(QuestCheckQuestFinished)
		MAKE_LUA_METHOD(QuestCheckExistEvent)
		MAKE_LUA_METHOD(CountMonsterQuestMain)
		MAKE_LUA_METHOD(CheckGiveSlot)
		MAKE_LUA_METHOD(CheckExchange)
		MAKE_LUA_METHOD(RunQuestExchange)
		MAKE_LUA_METHOD(RunRandomExchange)
		MAKE_LUA_METHOD(RunMiningExchange)
		MAKE_LUA_METHOD(SendNameChange)
		MAKE_LUA_METHOD(SendClanNameChange)
		MAKE_LUA_METHOD(SendTagNameChangePanel)
		...
		MAKE_LUA_METHOD(GiveSwitchPremium) //17.12.2020 Switch Premium Yukler Yuklemez 3 n birden goruntuluyecek sekilde yeni komut eklendi
		MAKE_LUA_METHOD(GiveClanPremium)
		MAKE_LUA_METHOD(GivePremiumItem)
		MAKE_LUA_METHOD(GetMonsterChallengeTime)
		MAKE_LUA_METHOD(GetMonsterChallengeUserCount)
		MAKE_LUA_METHOD(GetUnderTheCastleOpen)
		MAKE_LUA_METHOD(GetUnderTheCastleUserCount)
		MAKE_LUA_METHOD(GetJuraidMountainTime)
		...
		MAKE_LUA_METHOD(NpcEventSystem)
		MAKE_LUA_METHOD(JoinEvent)
		MAKE_LUA_METHOD(KillNpcEvent)
		MAKE_LUA_METHOD(isCswWinnerNembers)
		MAKE_LUA_METHOD(DelosCasttellanZoneOut)
		MAKE_LUA_METHOD(CycleSpawn)
		MAKE_LUA_METHOD(DrakiRiftChange)
		MAKE_LUA_METHOD(DrakiOutZone)
		MAKE_LUA_METHOD(DrakiTowerNpcOut)
		MAKE_LUA_METHOD(GetExpPercent)
		MAKE_LUA_METHOD(BeefEventLogin)
		MAKE_LUA_METHOD(GenieExchange)
		MAKE_LUA_METHOD(ClanNts)
		MAKE_LUA_METHOD(PerkUseItem)
		MAKE_LUA_METHOD(MonsterStoneQuestJoin)
	);
...
{
	CUser * pUser = Lua_GetUser();
	uint8 bNation = (uint8)Nation::NONE;
...
LUA_WRAPPER_USER_FUNCTION(RunRandomExchange);
LUA_WRAPPER_USER_FUNCTION(RunQuestExchange);
LUA_WRAPPER_USER_FUNCTION(RunMiningExchange);
...
LUA_WRAPPER_USER_FUNCTION(GetRebirthLevel);
LUA_WRAPPER_USER_FUNCTION(GetExp);
...
LUA_WRAPPER_USER_FUNCTION(GetCash);
...
LUA_WRAPPER_USER_FUNCTION(isPortuKurian);
...
LUA_WRAPPER_USER_FUNCTION(SendNpcKillID); //npckill
LUA_WRAPPER_USER_FUNCTION(GiveItemLua);
...
LUA_WRAPPER_USER_FUNCTION(RunGiveItemExchange);
LUA_WRAPPER_USER_FUNCTION(ExpChange);
LUA_WRAPPER_USER_FUNCTION(GiveLoyalty);
LUA_WRAPPER_USER_FUNCTION(GiveBalance);
...
LUA_WRAPPER_USER_FUNCTION(SendTagNameChangePanel);
...
LUA_WRAPPER_USER_FUNCTION(QuestCheckQuestFinished);
LUA_WRAPPER_USER_FUNCTION(QuestCheckExistEvent);
...
LUA_WRAPPER_USER_FUNCTION(GiveSwitchPremium); //17.12.2020 Switch Premium Yukler Yuklemez 3 n birden goruntuluyecek sekilde yeni komut eklendi
LUA_WRAPPER_USER_FUNCTION(GiveClanPremium);
LUA_WRAPPER_USER_FUNCTION(GivePremiumItem);
...
LUA_WRAPPER_USER_FUNCTION(NpcEventSystem);
LUA_WRAPPER_USER_FUNCTION(JoinEvent);
LUA_WRAPPER_USER_FUNCTION(KillNpcEvent);
LUA_WRAPPER_USER_FUNCTION(isCswWinnerNembers);
LUA_WRAPPER_USER_FUNCTION(DelosCasttellanZoneOut);
LUA_WRAPPER_USER_FUNCTION(CycleSpawn);
LUA_WRAPPER_USER_FUNCTION(DrakiRiftChange);
LUA_WRAPPER_USER_FUNCTION(DrakiOutZone);
LUA_WRAPPER_USER_FUNCTION(DrakiTowerNpcOut);
LUA_WRAPPER_USER_FUNCTION(GetExpPercent);
LUA_WRAPPER_USER_FUNCTION(GenieExchange);
LUA_WRAPPER_USER_FUNCTION(ClanNts);
LUA_WRAPPER_USER_FUNCTION(PerkUseItem);
LUA_WRAPPER_USER_FUNCTION(MonsterStoneQuestJoin);
/* method aliases */
...
_LUA_WRAPPER_USER_FUNCTION(CheckCash, GetCash);
...
_LUA_WRAPPER_USER_FUNCTION(CheckUnderTheCastleOpen, GetUnderTheCastleOpen);
_LUA_WRAPPER_USER_FUNCTION(CheckUnderTheCastleUserCount, GetUnderTheCastleUserCount);
_LUA_WRAPPER_USER_FUNCTION(CheckJuraidMountainTime, GetJuraidMountainTime);
_LUA_WRAPPER_USER_FUNCTION(SendNationTransfer, SendNationTransfer);
_LUA_WRAPPER_USER_FUNCTION(SendRepurchaseMsg, SendRepurchaseMsg);
_LUA_WRAPPER_USER_FUNCTION(RebirthBas, RebirthBas);
_LUA_WRAPPER_USER_FUNCTION(KingsInspectorList, KingsInspectorList);
_LUA_WRAPPER_USER_FUNCTION(CheckBeefEventLogin, BeefEventLogin);
...
LUA_FUNCTION(CastSkill)
{
	...
	if (pUser != nullptr)
	{
		CNpc * pNpc = g_pMain->GetNpcPtr(pUser->m_sEventNid, pUser->GetZoneID());
		if (pNpc != nullptr)
		{
