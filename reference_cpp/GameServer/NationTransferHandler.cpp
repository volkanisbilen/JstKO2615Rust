...
void CUser::SendNationTransfer()
{
	...
	if (!CheckExistItem(ITEM_NATION_TRANSFER)) {
		result.clear();
		result << uint8(NationOpenBox) << uint8(7);
		Send(&result);
		return;
	}
	...
	if (isTransformed()) {
		result.clear();
		result << uint8(NationOpenBox) << uint8(6);
		Send(&result);
		return;
	}
...
void CUser::ReqHandleNationChange(Packet & pkt)
{
	...
	float Px = (float)264.000000, Pz = (float)302.000000, Py = (float)4.590000;
	...
	for (int x = 0; x < kAccountCount; x++)
	{
		...
		if (bCurrentNation == (uint8)Nation::KARUS)
		{
	...
	for (int x = 0; x < kAccountCount; x++)
	{
		...
		if (bCurrentNation == (uint8)Nation::KARUS)
		{
			kNation = (uint8)Nation::ELMORAD;
			kClass = (pNation->sClass + 100);
		}
		else
		{
			kNation = (uint8)Nation::KARUS;
			kClass = (pNation->sClass - 100);
		}
	...
	NationTransferInsertLog(bCurrentNation, GetNation());;
...
void CUser::ReqSendNationTransfer()
{
	Packet result(WIZ_NATION_TRANSFER);
	uint16	CharClass = 0, nNum = 0;
	uint8	CharNation, CharRace = 0, AccountCount = 0, error = 1;
	std::string strCharID1, strCharID2, strCharID3, strCharID4;
	CSTLMap <_NATION_DATA> m_NationTransferArray;
	...
	for (int x = 0; x < AccountCount; x++)
	{
		...
		if (GetNation() == (uint8)Nation::KARUS)
		{
			...
			CharNation = (uint8)Nation::ELMORAD;
...
uint8 CUser::NationChange()
{
	...
	if (GetNewRace() == 0)
	{
		goDisconnect("a new gender could not be selected.", __FUNCTION__);
		return nRet;
	}
	...
	m_bTitle = 0;
	m_bRace = GetNewRace();
	m_sClass = GetNation() == (uint8)Nation::KARUS ? GetClass() + 100 : GetClass() - 100;
	m_bNation = GetNation() == (uint8)Nation::KARUS ? (uint8)Nation::ELMORAD : (uint8)Nation::KARUS;
