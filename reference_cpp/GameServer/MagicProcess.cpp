...
{
	if (g_pMain->m_IsMagicTableInUpdateProcess || pCaster == nullptr)
		return;

	if (pCaster->isPlayer() && !TO_USER(pCaster)->isInGame())
		return;

	MagicInstance instance;
	pkt >> instance.bOpcode >> instance.nSkillID;
	if (instance.nSkillID == 0 || (pCaster->isPlayer() && instance.bOpcode < 5 && TO_USER(pCaster)->m_bZoneChangeFlag))
		return;

	instance.pSkill = g_pMain->GetMagicPtr(instance.nSkillID);
	if (instance.pSkill.isnull())
	{
		//if (pCaster->isPlayer()) TO_USER(pCaster)->goDisconnect(string_format("wrong skill id. nskillid=%d", instance.nSkillID), __FUNCTION__);
		return;
	}
	...
	/*printf("%d %d %d ", instance.bOpcode, instance.sCasterID, instance.sTargetID);
	for (int i = 0; i < 7; i++)  printf("%d ", instance.sData[i]);
	printf("\n");*/

	if (instance.sTargetID != -1 && (instance.nSkillID == 109035 || instance.nSkillID == 110035
		|| instance.nSkillID == 209035 || instance.nSkillID == 210035))
	{
		instance.SendSkillFailed();
		return;
	}
	if (instance.sTargetID != instance.sCasterID && (instance.nSkillID == 109015 || instance.nSkillID == 110015
		|| instance.nSkillID == 209015 || instance.nSkillID == 210015))
	{
		instance.SendSkillFailed();
		return;
	}
...
void CMagicProcess::MagicPacketNpc(Packet& pkt, Unit* pCaster /*= nullptr*/)
{
	...
	if (g_pMain->m_IsMagicTableInUpdateProcess || !pCaster->isNPC())
		return;
	...
	if (instance.sCasterID != pCaster->GetID())
		return;
...
void CMagicProcess::MagicPacketBot(Packet& pkt, Unit* pCaster /*= nullptr*/)
{
	if (pCaster == nullptr)
		return;

	if (g_pMain->m_IsMagicTableInUpdateProcess || !pCaster->isBot())
		return;
...
bool CMagicProcess::UserRegionCheck(Unit* pSkillCaster, Unit* pSkillTarget, _MAGIC_TABLE pSkill, int radius, short mousex /*= 0*/, short mousez /*= 0*/) // botlara party 10 k gitmesi engellendi DeaFSezo
{
	if (!pSkillCaster || !pSkillTarget || pSkill.isnull())
		return false;
	...
	{
		if (TO_NPC(pSkillTarget)->GetType() == NPC_DESTROYED_ARTIFACT && !g_pMain->isCswWarActive())
			return false;
	}

	if (!pSkillTarget->isNPC()
		&& pSkillCaster->isPlayer()
		&& TO_USER(pSkillCaster)->isMage()
	...
	if (g_pMain->isCindirellaZone(pSkillCaster->GetZoneID())
		&& g_pMain->pCindWar.isON()
	...
	if (pSkillTarget->isNPC())
	{
		if (TO_NPC(pSkillTarget)->GetType() == NPC_TREE)
			...
			return false;
		else if (TO_NPC(pSkillTarget)->GetNation() == (uint8)Nation::NONE &&
	...
	if (pSkillTarget->isNPC() && g_pMain->m_AntiAfkList.size() && std::find(g_pMain->m_AntiAfkList.begin(), g_pMain->m_AntiAfkList.end(), TO_NPC(pSkillTarget)->GetProtoID()) != g_pMain->m_AntiAfkList.end())
	{
	...
	{
		// Friend skills can only affect non-hostile non-NPC units (including self).
		if (pSkillTarget->isNPC())
			return false;
	...
	{
		// Same as friend skills, but cannot target self.
		if (pSkillTarget->isNPC())
			return false;
	...
	case MORAL_PARTY:
	{
		...
		if (pSkillCaster->isPlayer())
		{
			...
			else if (pSkillTarget->isBot())
			{
				if (!TO_BOT(pSkillTarget)->isInParty())
					return (pSkillTarget == pSkillCaster);

				if (TO_BOT(pSkillTarget)->GetPartyID() != TO_USER(pSkillCaster)->GetPartyID())
					return (pSkillTarget == pSkillCaster);
			}
		}
		else if (pSkillCaster->isBot())
		{
			if (!TO_BOT(pSkillCaster)->isInParty())
				return (pSkillTarget == pSkillCaster);
			...
			if (pSkillTarget->isPlayer())
			{
				...
				if (TO_USER(pSkillTarget)->GetPartyID() != TO_BOT(pSkillCaster)->GetPartyID())
					return (pSkillTarget == pSkillCaster);
			}
			else if (pSkillTarget->isBot())
			{
				...
				if (TO_BOT(pSkillTarget)->GetPartyID() != TO_BOT(pSkillCaster)->GetPartyID())
					return (pSkillTarget == pSkillCaster);
			}
		}
	}
	...
	case MORAL_PARTY_ALL:		// Check that it's your party.
	{
		...
		if (pSkill.bType[0] == 3 && pSkill.bType[1] == 0)
		{
			...
			if (pType3 && pType3->bDuration != 0 && pType3->bDirectType == 1)
			{
		...
		if (pSkill.bType[0] == 8)
		{
			if (pSkillTarget->isPlayer())
			{
				if (pSkillTarget->GetMap()->isWarZone()
					&& (UNIXTIME - TO_USER(pSkillTarget)->m_tLastRegeneTime < CLAN_SUMMON_TIME))
					return false;
			}
		}
	...
	case MORAL_AREA_ALL:
		if (pSkillCaster->isNPC() || pSkillTarget->isNPC())
			return false;

		if (((pSkillCaster)->isInArena() && pSkillTarget->isInArena())
			|| (pSkillCaster->isInPVPZone() && pSkillTarget->isInPVPZone())
			|| (pSkillCaster->isInTempleEventZone() && pSkillTarget->isInTempleEventZone())
			|| (pSkillCaster->isInTempleQuestEventZone() && pSkillTarget->isInTempleQuestEventZone()))
			goto final_test;

		// Players cant attack other players in the safety area.
		if (pSkillCaster->isInEnemySafetyArea() && pSkillTarget->isInOwnSafetyArea())
			return false;
	...
	case MORAL_CLAN_ALL:
	{
		...
		if (pSkillCaster->isPlayer())
		{
			...
			{
				if (!TO_BOT(pSkillTarget)->isInClan())
					return false;

				if (TO_BOT(pSkillTarget)->GetClanID() != TO_USER(pSkillCaster)->GetClanID())
					return (pSkillTarget == pSkillCaster);
			}
		}
		else if (pSkillCaster->isBot())
		{
			if (!TO_BOT(pSkillCaster)->isInClan())
				return false;
			...
			if (pSkillTarget->isPlayer())
			{
				...
				if (TO_USER(pSkillTarget)->GetClanID() != TO_BOT(pSkillCaster)->GetClanID())
					return (pSkillTarget == pSkillCaster);
			}
			else			if (pSkillTarget->isBot())
			{
				...
				if (TO_BOT(pSkillTarget)->GetClanID() != TO_BOT(pSkillCaster)->GetClanID())
					return (pSkillTarget == pSkillCaster);
			}
		}
...
final_test:
	/*float mesafe = sqrtf(pSkillTarget->GetDistance(float(mousex), float(mousez)));
	if (radius && mesafe > radius)
		return false;*/
...
void CMagicProcess::RemoveStealth(Unit* pTarget, InvisibilityType bInvisibilityType)
{
	...
	if (bInvisibilityType != InvisibilityType::INVIS_DISPEL_ON_MOVE && bInvisibilityType != InvisibilityType::INVIS_DISPEL_ON_ATTACK)
		return;

	uint8 bType = (bInvisibilityType == InvisibilityType::INVIS_DISPEL_ON_MOVE ? 1 : 2);

	uint32 Num = 0;

	pTarget->m_buff9lock.lock();
	auto itr = pTarget->m_type9BuffMap.find(bType);
	if (itr == pTarget->m_type9BuffMap.end())
	{
		pTarget->m_buff9lock.unlock();
		return;
	}

	Num = itr->second.nSkillID;
	pTarget->m_buff9lock.unlock();

	MagicInstance instance;
	instance.pSkillCaster = pTarget;
	instance.nSkillID = Num;
...
{
	if (!pTarget || !pCaster || pSkill.isnull())
		return false;
	...
	if (!bIsRecastingSavedMagic && pTarget->m_buffMap.find(pType->bBuffType) != pTarget->m_buffMap.end())
		return false;
	...
	case BUFF_TYPE_AC:
		...
		break;
	case BUFF_TYPE_JACKPOT:
		if (pTarget->isPlayer() && pType->iNum == 501570) TO_USER(pTarget)->m_jackpotype = 1;
		else if (pTarget->isPlayer() && pType->iNum == 501571) TO_USER(pTarget)->m_jackpotype = 2;
		else if (pTarget->isPlayer() && pType->iNum == 501572) TO_USER(pTarget)->m_jackpotype = 3;
		break;
	case BUFF_TYPE_SIZE:
		if (pCaster->isPlayer())
		{
	...
	case BUFF_TYPE_STATS:
		if (pTarget->isPlayer())
		{
			TO_USER(pTarget)->SetStatBuffAdd(StatType::STAT_STR, pType->bStr);
			TO_USER(pTarget)->SetStatBuffAdd(StatType::STAT_STA, pType->bSta);
			TO_USER(pTarget)->SetStatBuffAdd(StatType::STAT_DEX, pType->bDex);
			TO_USER(pTarget)->SetStatBuffAdd(StatType::STAT_INT, pType->bIntel);
			TO_USER(pTarget)->SetStatBuffAdd(StatType::STAT_CHA, pType->bCha);
		}
		break;
	...
	case BUFF_TYPE_EXPERIENCE:
		if (pTarget->isPlayer() && pType->sExpPct > 100)
			TO_USER(pTarget)->m_sExpGainbuff11Amount += (pType->sExpPct - 100);

		/*if (pTarget->isPlayer() && pType->sSpecialAmount > 0)
			TO_USER(pTarget)->m_drop_scroll_amount += pType->sSpecialAmount;*/
		break;
	...
	case BUFF_TYPE_WEAPON_AC:
		...
		{
			if (pType->sAC == 0 && pType->sACPct > 0)
				TO_BOT(pTarget)->m_bPctArmourAc += (pType->sACPct - 100);
			else
				TO_BOT(pTarget)->m_sAddArmourAc += pType->sAC;
		}
		break;
	...
	case BUFF_TYPE_PREMIUM_MERCHANT:
		...
		if (pSkill.iNum == 495145)
		{
			...
			Packet slave(XSafe, uint8(XSafeOpCodes::PL_SLAVE_MERC));
			slave << uint8(1) << uint8(1);
			TO_USER(pTarget)->Send(&slave);

			Packet slaveUpTime(XSafe, uint8(XSafeOpCodes::PL_SLAVE_MERC));
	...
	case BUFF_TYPE_ATTACK_SPEED_ARMOR:
		if (pType->sAC < 0)
		{
			int16 testAc = 0;
			testAc -= pType->sAC;
			pTarget->m_sACSourAmount += testAc;
		}
		else
		{
			pTarget->m_sACAmount += pType->sAC;
	...
	case BUFF_TYPE_FREEZE:
		...
		{
			pTarget->m_bSpeedAmount = pType->bSpeed;
			TO_USER(pTarget)->m_bAbnormalType = AbnormalType::ABNORMAL_BLINKING;
			TO_USER(pTarget)->m_tBlinkExpiryTime = UNIXTIME + pType->sDuration;
			TO_USER(pTarget)->m_bCanUseSkills = false;
			TO_USER(pTarget)->m_bInvisibilityType = (uint8)InvisibilityType::INVIS_NONE;  // but players should. 
			TO_USER(pTarget)->StateChangeServerDirect(3, AbnormalType::ABNORMAL_BLINKING);
		}
		break;
	...
	case BUFF_TYPE_VARIOUS_EFFECTS:
		//... whatever the event item grants.
		...
		if (pTarget->isPlayer() && pType->sExpPct > 100)
			TO_USER(pTarget)->m_sExpGainbuff33Amount += (pType->sExpPct - 100);
	...
	case BUFF_TYPE_DEVIL_TRANSFORM: 
		...
		pTarget->m_sMagicAttackAmount = (pType->bMagicAttack - 100) / 2;
	...
	case BUFF_TYPE_HELL_FIRE_DRAGON:
		...
		pTarget->m_bAttackAmount += (pType->bAttack - 100);
		break;
	case BuffType::BUFF_TYPE_MAGIC_SPELL:
		if (pTarget->isPlayer())
			pTarget->m_sMagicAttackAmount += (pType->bMagicAttack - 100);
		break;
...
bool CMagicProcess::RemoveType4Buff(uint8 byBuffType, Unit* pTarget, bool bRemoveSavedMagic /*= true*/, bool bRecastSavedMagic /*= false*/)
{
	...
	auto pSkill = g_pMain->GetMagicPtr(itr->second.m_nSkillID);
	if (pSkill.isnull())
		return false;
	...
	if (bRemoveSavedMagic && pTarget->isPlayer() && pTarget->HasSavedMagic(pSkill.iNum))
		TO_USER(pTarget)->RemoveSavedMagic(pSkill.iNum);

	if (bRemoveSavedMagic&& pTarget->isBot() && pTarget->HasSavedMagic(pSkill.iNum))
		TO_BOT(pTarget)->RemoveSavedMagic(pSkill.iNum);

	if (itr->second.isBuff() && pTarget->m_buffCount)
		pTarget->m_buffCount--;

	CBot* pPriest = nullptr;
	CUser* pUser = nullptr;
	pUser = g_pMain->GetUserPtr(TO_USER(pTarget)->GetID());

	if (pUser != nullptr)
		pPriest = g_pMain->m_MapBotList.GetData(pUser->m_bUserPriestBotID);

	if (pSkill.iNum == 495146)
	{
		Packet pkt(XSafe, uint8(XSafeOpCodes::PL_SLAVEPRIEST));
		pkt << uint8(2);
		pUser->Send(&pkt);
	}

	if (pUser != nullptr && pSkill.iNum == 302166)
	{
		if (pUser->m_teamColour == TeamColourBlue || pUser->m_teamColour == TeamColourRed)
		{
			pUser->m_teamColour = TeamColourNone;
		}

		pUser->SendMyInfo();
		pUser->UserInOut(INOUT_OUT);
		pUser->RemoveRegion(pUser->GetNewRegionX(), pUser->GetNewRegionZ());

		pUser->UserInOut(INOUT_RESPAWN);
	...
	case BUFF_TYPE_AC:
		...
		break;
	case BUFF_TYPE_JACKPOT:
		if (pTarget->isPlayer()) TO_USER(pTarget)->m_jackpotype = 0;
		break;
	case BUFF_TYPE_SIZE:
		if (pTarget->isPlayer())
		{
	...
	case BUFF_TYPE_STATS:
		if (pTarget->isPlayer())
		{
			TO_USER(pTarget)->SetStatBuffRemove(StatType::STAT_STR, pType->bStr);
			TO_USER(pTarget)->SetStatBuffRemove(StatType::STAT_STA, pType->bSta);
			TO_USER(pTarget)->SetStatBuffRemove(StatType::STAT_DEX, pType->bDex);
			TO_USER(pTarget)->SetStatBuffRemove(StatType::STAT_INT, pType->bIntel);
			TO_USER(pTarget)->SetStatBuffRemove(StatType::STAT_CHA, pType->bCha);
		}
		break;
	...
	case BUFF_TYPE_BLIND:
		...
		{
			pTarget->m_bIsBlinded = false;
			TO_USER(pTarget)->SendUserStatusUpdate(UserStatus::USER_STATUS_POISON, UserStatusBehaviour::USER_STATUS_CURE);
		}
		break;
	...
	case BUFF_TYPE_FREEZE:
		...
		if (pTarget->isPlayer())
		{
			TO_USER(pTarget)->m_bAbnormalType = REGENE_NORMAL;
			TO_USER(pTarget)->m_bCanUseSkills = true;
			TO_USER(pTarget)->m_bInvisibilityType = (uint8)InvisibilityType::INVIS_NONE; // but players should. 
			pTarget->StateChangeServerDirect(3, AbnormalType::ABNORMAL_NORMAL);
			if (TO_USER(pTarget)->isTransformed()) pTarget->StateChangeServerDirect(3, TO_USER(pTarget)->m_sTransformSkillID);
		}
		break;
	...
	case BUFF_TYPE_FRAGMENT_OF_MANES:
		if (pTarget->isPlayer())TO_USER(pTarget)->BDWUserHasObtainedLoqOut();
		pTarget->m_bSpeedAmount = 100;
		break;
	...
	case BUFF_TYPE_VARIOUS_EFFECTS:
		...
		if (pTarget->isPlayer() && (pType->iNum == 501570 || pType->iNum == 501571 || pType->iNum == 501572)) TO_USER(pTarget)->m_jackpotype = 0;
		break;
	case BUFF_TYPE_PASSION_OF_SOUL:		// Passion of the Soul
		// Increase pet's HP by 120
		break;
	...
	case BUFF_TYPE_NP_DROP_NOAH:
		if (pTarget->isPlayer() && pType->sSpecialAmount > 0)
		{
			if (pType->sSpecialAmount > TO_USER(pTarget)->m_drop_scroll_amount)
				TO_USER(pTarget)->m_drop_scroll_amount = 0;
			else
				TO_USER(pTarget)->m_drop_scroll_amount -= pType->sSpecialAmount;
		}
		break;
	...
	if (pTarget->isPlayer())
	{
		if (pSkill.bMoral >= MORAL_ENEMY)
		{
			if (byBuffType == BUFF_TYPE_SPEED || byBuffType == BUFF_TYPE_SPEED2)
				TO_USER(pTarget)->SendUserStatusUpdate(UserStatus::USER_STATUS_SPEED, UserStatusBehaviour::USER_STATUS_CURE);
		...
		if (bRemoveSavedMagic)
		{
			...
			/*07.10.2020 Cry Echo Kararmaması Fix*/
			if (byBuffType == (uint8)BuffType::BUFF_TYPE_BATTLE_CRY)  TO_USER(pTarget)->SendItemMove(1, 1);
			/*07.10.2020 Cry Echo Kararmaması Fix*/
		}
...
bool CMagicProcess::IsBuff(_MAGIC_TYPE4* pType)
{
	...
	case BUFF_TYPE_HP_MP:
		if (pType->sMaxHP > 0 || pType->sMaxMP > 0)
			return true;
