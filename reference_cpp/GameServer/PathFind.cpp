...
CPathFind::CPathFind() : m_min_x(0), m_min_y(0), m_nMapSize(0)
{
...
void CPathFind::SetMap(int x, int y, C3DMap* pMap, int16 min_x, int16 min_y)
{
	m_vMapSize.cx = x;
	m_vMapSize.cy = y;
	m_pMap = pMap;
...
_PathNode* CPathFind::FindPath(int start_x, int start_y, int dest_x, int dest_y)
{
	...
	t_node->g = 0;
	t_node->h = (int)sqrt((double)(((int)start_x - (int)dest_x) * ((int)start_x - (int)dest_x) + ((int)start_y - (int)dest_y) * ((int)start_y - (int)dest_y)));
