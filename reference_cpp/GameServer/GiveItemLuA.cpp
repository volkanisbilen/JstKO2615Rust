	...
	uint32 nRobItemCount5)
{
	...
	_ITEM_TABLE pnGiveItemID1 = _ITEM_TABLE();
	_ITEM_TABLE pnGiveItemID2 = _ITEM_TABLE();
	_ITEM_TABLE pnGiveItemID3 = _ITEM_TABLE();
	_ITEM_TABLE pnGiveItemID4 = _ITEM_TABLE();
	_ITEM_TABLE pnGiveItemID5 = _ITEM_TABLE();
	_ITEM_TABLE pnGiveItemID6 = _ITEM_TABLE();
	_ITEM_TABLE pnGiveItemID7 = _ITEM_TABLE();
	_ITEM_TABLE pnGiveItemID8 = _ITEM_TABLE();
	...
	if (nGiveItemList.size() > 0)
	{
		foreach(itr, nGiveItemList)
		{
			...
			else
			{
				...
				if (pnGiveItemData1 == nullptr)
					return false;

				if (pnGiveItemData1->nNum != 0)
					bNewItem = false;
...
bool CUser::CheckExistItemLua(uint32 itemid, uint32 count)
{
	...
	{
		if (itemid == 379059000 && m_sItemArray[i].nNum == 479059000)
			return true;
...
bool CUser::RobItemLua(uint32 nItemID1, uint32 sCount1, uint32 nItemID2, uint32 sCount2, uint32 nItemID3, uint32 sCount3, uint32 nItemID4, uint32 sCount4, uint32 nItemID5, uint32 sCount5)
{
	...
	_ITEM_TABLE pTable1 = _ITEM_TABLE();
	_ITEM_TABLE pTable2 = _ITEM_TABLE();
	_ITEM_TABLE pTable3 = _ITEM_TABLE();
	_ITEM_TABLE pTable4 = _ITEM_TABLE();
	_ITEM_TABLE pTable5 = _ITEM_TABLE();
	Packet result;
