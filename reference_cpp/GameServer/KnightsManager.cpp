...
void CKnightsManager::PacketProcess(CUser *pUser, Packet & pkt)
{
	...
	case KnightsPacket::KNIGHTS_DESTROY:
		...
		break;
	case KnightsPacket::KNIGHTS_ADMIT:// done
	...
	case KnightsPacket::KNIGHTS_UPDATEMEMO:
		...
		break;
	case KnightsPacket::KNIGHTS_HANDOVER_VICECHIEF_LIST:
	...
	case KnightsPacket::KNIGHTS_POINT_METHOD:
		...
		break;
	case KnightsPacket::KNIGHTS_VS_LIST:
		KnightsVsLoginList(pUser);
		break;
	case KnightsPacket::KNIGHTS_LADDER_POINTS:
		KnightsLadderPointList(pUser, pkt);
	break;
	case KnightsPacket::KNIGHTS_CLAN_BONUS:
	break;
	default:
...
void CKnightsManager::KnightsFlagsList(CUser* pUser, Packet& pkt)
{
	/*if (pUser == nullptr || pUser->m_sendknightflaglist)
		return;

	pUser->m_sendknightflaglist = true;*/

	Packet newpkt(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_UNK1));
	for (int nation = KARUS_ARRAY; nation <= ELMORAD_ARRAY; nation++) {
		...
		foreach(itr, mlist) {
			...
			auto* pKnights = g_pMain->GetClanPtr(*itr);
			if (pKnights && pKnights->m_byNation == nation + 1)
				newpkt << int16(++i) << pKnights->GetID() << pKnights->m_strName << pKnights->m_sMarkVersion;
		}

		for (int k = i; k < 5; k++)
			newpkt << int16(k) << int16(-1) << "" << uint16(0);
	}
	pUser->Send(&newpkt);
}
...
void CKnightsManager::KnightsVsLoginList(CUser* pUser)
{
	...
	Packet TournamentClanPacket(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_VS_LIST));
...
{
	if (pUser == nullptr || pUser->isTrading() || pUser->isMerchanting() || pUser->isSellingMerchantingPreparing())
		return;
	...
	if (idname.empty() || idname.size() > MAX_ID_SIZE
		...
		ret_value = InvalidClanName; // Invalid clan name or invalid clan name
	else if (pUser->GetClanID() > NO_CLAN)
		ret_value = UserAlreadyInClan; // user already in a clan.
	else if (g_pMain->m_nServerGroup == 2)
		ret_value = InvalidServer; // cannot create clan in the second servers.
	else if (!pUser->GetMap()->canUpdateClan())
		ret_value = UnallowedZones; // cannot create in unallowed zones
	else if (pUser->GetLevel() < g_pMain->ClanCreateMinLevel)
		ret_value = MinLevelCreateClan; // level is below 20
	else if (!pUser->hasCoins(g_pMain->ClanCreateCoins))
		ret_value = HaveCoinsDont; // dont have enough coins to create a clan
	else if (pUser->GetLoyalty() < g_pMain->ClanCreateMinNationalPoints)
		ret_value = MinNpError; // dont have enough national points to create a clan

	// JstKO Yeni Klan Kurabilmek Notice Eklendi 01.01.2025
	if (ret_value == MinLevelCreateClan) // JstKO Klan Kurabilmek Level Notice Eklendi 01.01.2025
	{
		g_pMain->SendYesilNotice(pUser, string_format("[Clan Bilgilendirme 1] = Clan Kurabilmek ��in En Az [%d] Level'niz Olmas� Gereklidir", g_pMain->ClanCreateMinLevel));
		return;
	}

	if (ret_value == HaveCoinsDont) // JstKO Klan Kurabilmek Para Notice Eklendi 01.01.2025
	{
		g_pMain->SendYesilNotice(pUser, string_format("[Clan Bilgilendirme 2] = Clan Kurabilmek ��in En Az [%d] Para'niz Olmas� Gereklidir", g_pMain->ClanCreateCoins));
		return;
	}

	if (ret_value == MinNpError) // JstKO Klan Kurabilmek NP Notice Eklendi 01.01.2025
	{
		g_pMain->SendYesilNotice(pUser, string_format("[Clan Bilgilendirme 3] = Clan Kurabilmek ��in En Az [%d] NP'niz Olmas� Gereklidir", g_pMain->ClanCreateMinNationalPoints));
		return;
	}
	// JstKO Yeni Klan Kurabilmek Notice Eklendi The End 01.01.2025

	if (ret_value == 0)
	{
		int knightindex = GetKnightsIndex(pUser->m_bNation);
		if (knightindex >= 0) {
			result << uint8(g_pMain->pServerSetting.AutoRoyalG1 ? ClanTypeFlag::ClanTypeRoyal1 : ClanTypeFlag::ClanTypeTraining)
				<< (uint16)knightindex << pUser->GetNation() << idname << pUser->GetName();
...
void CKnightsManager::KnightsJoin(CUser* pUser, Packet& pkt)
{
	...
	result << uint8(KnightsPacket::KNIGHTS_JOIN);
	if (pUser->GetMap() == nullptr
		|| !pUser->GetMap()->canUpdateClan()) {
		result << uint8(12);
		pUser->Send(&result);
		return;
	}
	...
	CUser* pTUser = g_pMain->GetUserPtr(TargetID);
	if (pTUser == nullptr
		|| !pTUser->isInGame()
		|| TargetID >= MAX_USER) {
		result << uint8(2);
	...
	pTUser->m_bKnightsReq = pUser->GetClanID();
	result.clear();
	result << uint8(KnightsPacket::KNIGHTS_JOIN_REQ) << uint8(1)
		<< pUser->GetSocketID() << pUser->GetClanID() << pKnights->m_strName;
	pTUser->Send(&result);

	result.clear();
	result << uint8(KnightsPacket::KNIGHTS_JOIN) << uint8(0);
	pUser->Send(&result);
}
#pragma endregion
...
void CKnightsManager::KnightsWithdraw(CUser *pUser, Packet & pkt)
{
	...
	uint8 Type = uint8(pUser->isClanLeader() ? KnightsPacket::KNIGHTS_DESTROY : KnightsPacket::KNIGHTS_WITHDRAW);
	Packet result(WIZ_KNIGHTS_PROCESS, Type);

	if (!pUser->isInClan() || !pUser->m_pKnightsUser
		|| pUser->GetFame() == COMMAND_CAPTAIN || !pUser->GetMap()) {

		if (Type == (uint8)KnightsPacket::KNIGHTS_WITHDRAW) {
			result << uint8(2);
			pUser->Send(&result);
		}
		return;
	...
	/*if (pUser->isClanLeader() && !pUser->GetMap()->canUpdateClan()) {
		if (Type == (uint8)KnightsPacket::KNIGHTS_WITHDRAW) {
			result << uint8(12);
			pUser->Send(&result);
		}
...
void CKnightsManager::KnightsRemove(CUser *pUser, Packet & pkt)
{
	...
	do
	{
		...
		CUser *pTUser = g_pMain->GetUserPtr(strUserID, NameType::TYPE_CHARACTER);
		if (pTUser != nullptr)
		{
...
void CKnightsManager::KnightsAllMember(CUser *pUser)
{
	...
	if (bResult == 1)
	{
		...
		count = pKnights->GetKnightsAllMembers(result, pktSize, pUser);
...
void CKnightsManager::KnightsCurrentMember(CUser *pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_CURRENT_REQ));
	CKnights *pKnights = nullptr;
	if (!pUser->isInClan()
	...
	result << uint8(1) << pKnights->m_strChief << page;
	...
	foreach_stlmap_nolock(i, pKnights->m_arKnightsUser)
		if (i->second) m_userlist.push_back(i->second->strUserName);
...
void CKnightsManager::KnightsAllList(CUser *pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_ALLLIST_REQ));
...
void CKnightsManager::KnightsRequestSymbolVersion(CUser* pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_MARK_VERSION_REQ));
	int16 sFailCode = 1;
...
void CKnightsManager::KnightsGetSymbol(CUser* pUser, uint16 sClanID)
{
	...
	result	<< uint8(KnightsPacket::KNIGHTS_MARK_REQ) << uint16(1); // success
...
void CKnightsManager::KnightsHandoverList(CUser *pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_HANDOVER_VICECHIEF_LIST));
	result << isClanLeader;
	...
	if (g_pMain->GetUserPtr(pKnights->m_strViceChief_1, NameType::TYPE_CHARACTER))
	{
		result << pKnights->m_strViceChief_1;
		ViceChiefCount++;
	}

	if (g_pMain->GetUserPtr(pKnights->m_strViceChief_2, NameType::TYPE_CHARACTER))
	{
		result << pKnights->m_strViceChief_2;
		ViceChiefCount++;
	}

	if (g_pMain->GetUserPtr(pKnights->m_strViceChief_3, NameType::TYPE_CHARACTER))
	{
...
void CKnightsManager::KnightsHandoverReq(CUser *pUser, Packet & pkt)
{
	...
	if (pUser->isClanLeader())
	{
		...
		result << uint8(KnightsPacket::KNIGHTS_HANDOVER) << pUser->GetClanID() << pUser->GetName() << pTUser->GetName();
		g_pMain->AddDatabaseRequest(result, pUser);
		return;
fail_handover:
		result << uint8(KnightsPacket::KNIGHTS_HANDOVER_REQ) << uint8(3);
		pUser->Send(&result);
		return;
	}
...
void CKnightsManager::KnightsPointMethodModify(CUser *pUser, Packet & pkt)
{
	...
	if (pKnights->m_byFlag >= (uint8)ClanTypeFlag::ClanTypeAccredited5 && pUser->GetName() == pKnights->m_strChief)
	{
		uint8 method  = subCode != 0 ? subCode - 1 : pKnights->m_sClanPointMethod;
		Packet DB_pkt(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_POINT_METHOD));
	...
	Packet result(WIZ_KNIGHTS_PROCESS, (uint8)KnightsPacket::KNIGHTS_POINT_METHOD);
...
{
	/*printf("ListTop10Clans giris yapti return var\n");*/
	return;

	if (pUser == nullptr)
		return;

	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_TOP10));
	result << uint16(0);
...
{
	int knightindex = 1;
	if (nation == (uint8)Nation::ELMORAD) knightindex = 15001;

	g_pMain->m_KnightsArray.m_lock.lock();
	foreach_stlmap_nolock(itr, g_pMain->m_KnightsArray)
	{
		if (itr->second == nullptr) continue;
		if (knightindex < itr->second->GetID())
		{
			if (nation == (uint8)Nation::KARUS && itr->second->GetID() >= 15000)
				continue;
	...
	if (knightindex + 1 > 32000)
		return -1;

	knightindex++;

	if ((nation == (uint8)Nation::KARUS && knightindex >= 15000) || (nation == (uint8)Nation::ELMORAD && knightindex < 15000))
		return -1;

	if (g_pMain->GetClanPtr(knightindex))
		return -1;
...
bool CKnightsManager::AddKnightsUser(int index, std::string strAccountID, std::string & strUserID, std::string & strMemo, uint8 bFame, uint16 sClass, uint8 bLevel, int32 lastlogin, int32 Loyalty, uint32 LoyaltyMonthly)
{
	CKnights * pKnights = g_pMain->GetClanPtr(index);
	if (pKnights == nullptr)
		return false;

	if (!pKnights->AddUser(strAccountID,strUserID, bFame, sClass, bLevel, lastlogin, Loyalty, LoyaltyMonthly))
		return false;
	...
	if (pKnights->m_byFlag >= (uint8)ClanTypeFlag::ClanTypeAccredited5) pKnights->m_byGrade = 1;
	else pKnights->m_byGrade = g_pMain->GetKnightsGrade(pKnights->m_nPoints);
...
void CKnightsManager::UpdateKnightsGrade(uint16 sClanID, uint8 byFlag)
{
	...
	if (byFlag == (uint8)ClanTypeFlag::ClanTypeTraining)
		pClan->m_sCape = -1;
	else if (byFlag == (uint8)ClanTypeFlag::ClanTypePromoted)
		pClan->m_sCape = 0;
...
void CKnightsManager::DonateNPReq(CUser * pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_POINT_REQ));
	result	<< uint8(1) 
...
void CKnightsManager::DonateNP(CUser *pUser, Packet & pkt)
{
	...
	CKnights * pKnights = g_pMain->GetClanPtr(pUser->GetClanID());
	if (pKnights == nullptr
		|| pKnights->m_byFlag < (uint8)ClanTypeFlag::ClanTypeAccredited5)
		return;
...
void CKnightsManager::DonationList(CUser *pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_DONATION_LIST));
	uint8 count = 0;
	size_t wpos = result.wpos();
	result << count;
	...
	case 2:
		{
			...
			result << uint8(KnightsPacket::KNIGHTS_UPDATEMEMO) << type << pKnights->GetAllianceID() << pUser->GetName() << StrAllianceNotice;
			g_pMain->AddDatabaseRequest(result);

			result.Initialize(WIZ_KNIGHTS_PROCESS);
			result.DByte();
			result << uint8(KnightsPacket::KNIGHTS_UPDATEMEMO) << type << uint8(1) << StrAllianceNotice;
	...
	case 3:
		{
			...
			// Tell the database to update the strAllianceNotice.
			result.Initialize(WIZ_KNIGHTS_PROCESS);
			result << uint8(KnightsPacket::KNIGHTS_UPDATEMEMO) << type << pUser->GetClanID() << pUser->GetName() << StrUserMemo;
			g_pMain->AddDatabaseRequest(result);

			result.Initialize(WIZ_KNIGHTS_PROCESS);
			result.DByte();
			result << uint8(KnightsPacket::KNIGHTS_UPDATEMEMO) << type << uint8(1) << StrUserMemo;
			pKnights->Send(&result);
		}
		break;
	case 6:
...
error_title:
			result.Initialize(WIZ_KNIGHTS_PROCESS);
			result << uint8(KnightsPacket::KNIGHTS_UPDATEMEMO) << type << uint8(bResult) << username << title;
...
#pragma region CKnights::ClanPreAktifEdildi()

void CKnights::ClanPreAktifEdildi()
{

	Packet result;
	std::string updateNotice = string_format("[Clan Premium System] = Clan Premium Aktif Edildi");
	ChatPacket::Construct(&result, (uint8)ChatType::KNIGHTS_CHAT, &updateNotice);
	Send(&result);
...
void CKnightsManager::KnightsLadderPointList(CUser* pUser, Packet& pkt)
{
	if (pUser == nullptr || !pUser->isInClan()) return;

	auto* pKnights = g_pMain->GetClanPtr(pUser->GetClanID());
	if (pKnights == nullptr) return;

	uint16 count = 0;
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_LADDER_POINTS));
	result << count;//
	if (pKnights->m_ladlistime > UNIXTIME) {
		if (pKnights->m_ladlist.empty())goto show_reset;
		foreach(list, pKnights->m_ladlist) { result << list->name << list->loyalty; count++; }
		result.put(1, count);
		pUser->Send(&result);
		return;
	}
...
show_reset:
	pKnights->m_ladlistime = UNIXTIME + 60;
	pKnights->m_ladlist.clear();
	foreach_stlmap(user, pKnights->m_arKnightsUser) { auto* p = user->second; if (p) pKnights->m_ladlist.push_back(_ladderpointlist(p->strUserName, p->LoyaltyMonthly)); }
	if (pKnights->m_ladlist.empty()) return;
	std::sort(pKnights->m_ladlist.begin(), pKnights->m_ladlist.end(), [](auto const& a, auto const& b) { return a.loyalty > b.loyalty; });
	foreach(list, pKnights->m_ladlist) { result << list->name << list->loyalty; count++; }
	result.put(1, count);
	pUser->Send(&result);
}

#pragma region KnightsClanBuffUpdate(bool sign, CKnights *pmyknights)
void CUser::KnightsClanBuffUpdate(bool sign, CKnights* pmyknights)
{
	if (!isInClan() || isInAutoClan())
		return;

	Packet newpkt(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_CLAN_BONUS));
	...
	if (pKnights == nullptr) { newpkt << uint16(0); Send(&newpkt); return; }

	if (sign)
		pKnights->m_bOnlineNembers++;
	else if (pKnights->m_bOnlineNembers > 0)
		pKnights->m_bOnlineNembers--;

	if (pKnights->m_bOnlineNembers > MAX_CLAN_USERS)
		pKnights->m_bOnlineNembers = MAX_CLAN_USERS;

	if (pKnights->m_bOnlineNembers >= 5)
		pKnights->m_bOnlineNpCount = (int)ceil((double(pKnights->m_bOnlineNembers * 10) / 100));

	if (pKnights->m_bOnlineNembers >= 5)
		pKnights->m_bOnlineExpCount = 15 + pKnights->m_bOnlineNembers;

	if (pKnights->m_bOnlineExpCount > 65)
		pKnights->m_bOnlineExpCount = 65;

	if (pKnights->m_bOnlineNpCount > 5)
		pKnights->m_bOnlineNpCount = 5;

	newpkt << pKnights->m_bOnlineNembers;
	pKnights->Send(&newpkt);
}
#pragma endregion