...
uint8 CUser::GetUserDailyOp(uint8 type)
{
	...
	{
		if (type == (uint8)DailyOperationsOpCode::DAILY_CHAOS_MAP)
			nUnixTime = int(itr->second->ChaosMapTime);
		else if (type == (uint8)DailyOperationsOpCode::DAILY_USER_RANK_REWARD)
			nUnixTime = int(itr->second->UserRankRewardTime);
		else if (type == (uint8)DailyOperationsOpCode::DAILY_USER_PERSONAL_RANK_REWARD)
			nUnixTime = int(itr->second->PersonalRankRewardTime);
		else if (type == (uint8)DailyOperationsOpCode::DAILY_KING_WING)
			nUnixTime = int(itr->second->KingWingTime);
		else if (type == (uint8)DailyOperationsOpCode::DAILY_WARDER_KILLER_WING1)
			nUnixTime = int(itr->second->WarderKillerTime1);
		else if (type == (uint8)DailyOperationsOpCode::DAILY_WARDER_KILLER_WING2)
			nUnixTime = int(itr->second->WarderKillerTime2);
		else if (type == (uint8)DailyOperationsOpCode::DAILY_KEEPER_KILLER_WING)
			nUnixTime = int(itr->second->KeeperKillerTime);
		else if (type == (uint8)DailyOperationsOpCode::DAILY_USER_LOYALTY_WING_REWARD)
