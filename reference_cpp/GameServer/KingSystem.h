...
struct _KING_NOMINATION_LIST
{
	std::string strNominator;
...
typedef std::map<std::string, _KING_NOMINATION_LIST *, ci_compare> KingNominationList;
...
public:
	...
	// Generates a list of the King candidancies eligible to be a King.
	void DetermineKingCandidaciesList();
	...
	void InsertNominee(std::string & strNominator, std::string & strNominee, uint16 sNomineeKnights);
	void CandidacyNoticeBoard(CUser * pUser, Packet & pkt);
	void ElectionPoll(CUser * pUser, Packet & pkt);
	void ShowElectionList(CUser * pUser);
	void CandidacyResign(CUser * pUser, Packet & pkt);
	void VoteForKing(std::string & strVoterAccountID, std::string & strVoterUserID, std::string & strNominee);
	void GetElectionResults();
	void AssignNewKingAndSenators();
	...
	void KingNationIntro(CUser * pUser, Packet & pkt);
	...
	static void HandleDatabaseRequest_Tax(CUser * pUser, Packet & pkt);
	static void HandleDatabaseRequest_NationIntro(CUser * pUser, Packet & pkt);
	...
	uint8	m_byNextDay;
	uint8	m_byNextHour;
	uint8	m_byNextMinute;
	bool    m_bElectionUnderProgress;
	...
	uint16 m_sKingClanID;
	std::string m_strNewKingName;
	std::string m_strImRequestID;

	std::recursive_mutex m_KingSystemlock;
	...
	*/
	...
	/**
	* @brief	List of nominations still in the running for King.
	* 			Candidates for election must be senators,
	* 			so this data just refers to the above senator list.
	*/
	KingNominationList m_nominationList;
