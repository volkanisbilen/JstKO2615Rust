...
public:
	...
	static void KnightsLadderPointList(CUser* pUser, Packet& pkt);
	...
	static void KnightsFlagsList(CUser* pUser, Packet& pkt);
	...
	static bool AddKnightsUser(int index, std::string strAccountID, std::string & strUserID, std::string & strMemo, uint8 bFame, uint16 sClass, uint8 bLevel, int32 lastlogin, int32 Loyalty,uint32 LoyaltyMonthly);
	...
	static void GameLoadUpdateKnights();
