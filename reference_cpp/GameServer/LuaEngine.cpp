...
DEFINE_LUA_FUNCTION_TABLE(g_globalFunctions,
	...
	MAKE_LUA_FUNCTION(RunQuestExchange)
	MAKE_LUA_FUNCTION(RunRandomExchange)
	MAKE_LUA_FUNCTION(RunMiningExchange)
	...
	MAKE_LUA_FUNCTION(QuestCheckQuestFinished)
	MAKE_LUA_FUNCTION(QuestCheckExistEvent)
	...
	MAKE_LUA_FUNCTION(GetRebirthLevel)
	MAKE_LUA_FUNCTION(GetExp)
	...
	MAKE_LUA_FUNCTION(SendTagNameChangePanel)
	...
	MAKE_LUA_FUNCTION(GiveSwitchPremium) //17.12.2020 Switch Premium Yukler Yuklemez 3 n birden goruntuluyecek sekilde yeni komut eklendi
	MAKE_LUA_FUNCTION(GiveClanPremium)
	MAKE_LUA_FUNCTION(GivePremiumItem)
	MAKE_LUA_FUNCTION(RollDice)
	MAKE_LUA_FUNCTION(CheckMonsterChallengeTime)
	MAKE_LUA_FUNCTION(CheckMonsterChallengeUserCount)
	MAKE_LUA_FUNCTION(CheckUnderTheCastleOpen)
	MAKE_LUA_FUNCTION(CheckUnderTheCastleUserCount)
	MAKE_LUA_FUNCTION(CheckJuraidMountainTime)
	...
	MAKE_LUA_FUNCTION(SendNationTransfer)
	MAKE_LUA_FUNCTION(RebirthBas)
	MAKE_LUA_FUNCTION(SendRepurchaseMsg)
	MAKE_LUA_FUNCTION(NpcEventSystem)
	MAKE_LUA_FUNCTION(JoinEvent)
	MAKE_LUA_FUNCTION(KillNpcEvent)
	MAKE_LUA_FUNCTION(KingsInspectorList)
	MAKE_LUA_FUNCTION(isCswWinnerNembers)
	MAKE_LUA_FUNCTION(DelosCasttellanZoneOut)
	MAKE_LUA_FUNCTION(CycleSpawn)
	MAKE_LUA_FUNCTION(DrakiRiftChange)
	MAKE_LUA_FUNCTION(DrakiOutZone)
	MAKE_LUA_FUNCTION(DrakiTowerNpcOut)
	MAKE_LUA_FUNCTION(GetExpPercent)
	MAKE_LUA_FUNCTION(CheckBeefEventLogin)
	MAKE_LUA_FUNCTION(GenieExchange)
	MAKE_LUA_FUNCTION(ClanNts)
	MAKE_LUA_FUNCTION(PerkUseItem)
	MAKE_LUA_FUNCTION(CheckCash)
	MAKE_LUA_FUNCTION(MonsterStoneQuestJoin)

);
CLuaEngine::CLuaEngine() : m_lock(new std::recursive_mutex())
{
}
...
bool CLuaEngine::ExecuteScript(CUser * pUser, CNpc * pNpc, int32 nEventID, int8 bSelectedReward, const char * filename)
{
	...
	if (itr == m_scriptMap.end())
	{
		...
		Packet sNotice(WIZ_CHAT, uint8(ChatType::PUBLIC_CHAT));
		...
		result = SelectAvailableScript()->ExecuteScript(pUser, pNpc != nullptr ? pNpc : nullptr, nEventID, bSelectedReward,
			filename, bytecode);
