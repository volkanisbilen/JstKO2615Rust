...
bool CGameServerDlg::ChaosStoneLoad()
{
	...
	{
		foreach_stlmap(itr, g_pMain->m_ChaosStoneRespawnCoordinateArray)
		{
...
void CNpc::SpecialStoneDeath(CUser* pUser)
{
	...
	std::map<uint32, SpecialData> m_specialSelect;
	foreach(itr, g_pMain->m_SpecialStonearray)
	{
		SPECIAL_STONE* pData = itr->second;
		if (pData->MainNpcID != GetProtoID()
		...
		SpecialData data;
		data.sCount = pData->sCount;
		data.NpcMonster = pData->NpcMonster;

		m_specialSelect.insert(std::make_pair(pData->NpcID, data));
	}
#if 0
	int rand = myrand(1, (int)m_specialSelect.size()); 
	int say = 0;
	...
	foreach(itr, m_specialSelect) 
	{
		uint32 npcID = itr->first;
		SpecialData& data = itr->second;
		g_pMain->SpawnEventNpc(npcID, data.NpcMonster, GetZoneID(), GetX(), GetY(), GetZ(), 1, CHAOS_STONE_MONSTER_RESPAWN_RADIUS, CHAOS_STONE_MONSTER_LIVE_TIME, 0, -1, GetEventRoom(), 0, 1, 0, SpawnEventType::ChaosStoneSummon);
...
bool CDBAgent::LoadChaosStoneFamilyStage()
{
	...
	if (!dbCommand->Execute(_T("{CALL LOAD_CHAOS_STONE_STAGE2369}")))
#elif GAME_SOURCE_VERSION  == 1098
	if (!dbCommand->Execute(_T("{CALL LOAD_CHAOS_STONE_STAGE1098}")))
#elif GAME_SOURCE_VERSION  == 1534
	if (!dbCommand->Execute(_T("{CALL LOAD_CHAOS_STONE_STAGE1534}")))
#endif
	{
