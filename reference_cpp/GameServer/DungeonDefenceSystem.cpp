...
void CUser::DungeonDefenceSign()
{
	...
	for (int i = 0; i < MAX_PARTY_USERS; i++)
	{
		...
		if (pUser->ZoneChange(ZONE_DUNGEON_DEFENCE, 0.f, 0.f, LeaderEventRoom)) {
...
void CGameServerDlg::DungeonDefenceUserisOut(uint16 EventRoom)
{
	...
	std::vector< _DUNGEON_DEFENCE_ROOM_USER_LIST> mlist;
	...
	foreach(itr, mlist)
	{
		for (int i = 0; i < MAX_PARTY_USERS; i++)
		{
			CUser* pUser = g_pMain->GetUserPtr(itr->strUserID[i], NameType::TYPE_CHARACTER);
			if (pUser == nullptr
				|| !pUser->isInGame()
...
void CGameServerDlg::SendDungeonDefenceDetail(uint16 EventRoom)
{
	...
	case 1:
	{
		...
		Send_Zone(&x, ZONE_DUNGEON_DEFENCE, nullptr, (uint8)Nation::ALL, EventRoom);
	}
	break;
	case 2:
	{
...
uint8 CGameServerDlg::DungeonDefenceSelectStage(uint16 EventRoom)
{
	...
	{
		_DUNGEON_DEFENCE_STAGE_LIST* pStageList = itr->second;
		if (pStageList == nullptr)
			continue;

		if (pStageList->sStageID == pRoomInfo->m_DefenceStageID &&
			pStageList->Difficulty == pRoomInfo->m_DefenceDifficulty)
			return pStageList->sStageID;
	}
...
void CUser::DungeonDefenceSendFinishTimer()
{
	...
	g_pMain->Send_Zone(&result, ZONE_DUNGEON_DEFENCE, nullptr, (uint8)Nation::ALL, GetEventRoom());
...
void CNpc::DungeonDefenceProcess(CUser *pUser)
{
	...
	if (FullMoonRiftJevel1)
		pUser->GiveItem("Full Moon Rift", MONSTER_RIFT_JEWEL, 1);
	else if (FullMoonRiftJevel2)
		pUser->GiveItem("Full Moon Rift", MONSTER_RIFT_JEWEL, 2);
	else
		pUser->GiveItem("Full Moon Rift", MONSTER_RIFT_JEWEL, 1);
	...
	if (pUser->isInParty())
	{
		...
		for (int i = 0; i < MAX_PARTY_USERS; i++)
		{
			...
			if (pUser->GetName() == UserParty->GetName())
				UserParty->GiveItem("Full Moon Rift", MONSTER_COIN_ITEM, 2);
			else
				UserParty->GiveItem("Full Moon Rift", MONSTER_COIN_ITEM, 1);
			...
			if (GetProtoID() == 9931
				...
				|| GetProtoID() == 9959
				|| GetProtoID() == 9971)
				UserParty->GiveItem("Full Moon Rift", LUNAR_ORDER_TOKEN, 1);
		}
	}
	else
	{
		//monster coin
		pUser->GiveItem("Full Moon Rift", MONSTER_COIN_ITEM, 2);
		...
		if (GetProtoID() == 9931
			...
			|| GetProtoID() == 9959
			|| GetProtoID() == 9971)
			pUser->GiveItem("Full Moon Rift", LUNAR_ORDER_TOKEN, 1);
	}
}
#pragma endregion
