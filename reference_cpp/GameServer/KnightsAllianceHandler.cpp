...
{
	_KNIGHTS_ALLIANCE * pMainAlliance = g_pMain->GetAlliancePtr(pMainClan->GetAllianceID()), * pTargetAlliance = g_pMain->GetAlliancePtr(pTargetClan->GetAllianceID());
...
void CKnightsManager::KnightsAllianceCreate(CUser* pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_ALLY_CREATE));
...
{
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_ALLY_REQ));

	if (pUser == nullptr)
		return;

	if (pUser->isDead() || !pUser->isClanLeader() || !pUser->isInGame())
		pUser->Send(&result);

	CKnights* pMainClan = g_pMain->GetClanPtr(pUser->GetClanID());
	if (pMainClan == nullptr || pMainClan->m_byFlag <= 1) {
		result << uint8(0);
		pUser->Send(&result);
		return;
	}
	...
	uint16 TargetClanLeaderID = pkt.read<uint16>();
	CUser* pTargetUser = g_pMain->GetUserPtr(TargetClanLeaderID);
	if (pTargetUser == nullptr || pUser->GetNation() != pTargetUser->GetNation()
		|| !pTargetUser->isClanLeader() || !pTargetUser->isInGame()) {
		result << uint8(0);
		pUser->Send(&result);
		return;
	}
	...
	CKnights* pTargetClan = g_pMain->GetClanPtr(pTargetUser->GetClanID());
	if (pTargetClan == nullptr || pTargetClan->isInAlliance()) {
		result << uint8(0);
		pUser->Send(&result);
		return;
	}
	...
	if (pAlliance != nullptr) {
		if (pAlliance->sMercenaryClan_1 > NO_CLAN
...
void CKnightsManager::KnightsAllianceRequest(CUser* pUser, Packet & pkt)
{
	...
	// scoped
	{
		Packet ally_db(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_ALLY_INSERT));
...
void CKnightsManager::KnightsAllianceRemove(CUser* pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_ALLY_REMOVE));  // ally disband or leaving from the alliance
...
void CKnightsManager::KnightsAlliancePunish(CUser* pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_ALLY_PUNISH)); // knights ban
...
void CKnightsManager::KnightsAllianceList(CUser* pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_ALLY_LIST));
...
void CKnightsManager::ReqKnightsAllianceCreate(CUser* pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_ALLY_INSERT));
...
void CKnightsManager::ReqKnightsAllianceRemove(CUser* pUser, Packet & pkt)
{
	...
	if(pClan->GetID() == pAlliance->sMainAllianceKnights) // we are destroying the alliance.
	{
		...
		Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_ALLY_REMOVE));
...
void CKnightsManager::ReqKnightsAlliancePunish(CUser* pUser, Packet & pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_ALLY_PUNISH));
	result << uint8(1) << pMainClan->GetID() << pTargetClan->GetID() << pMainClan->GetCapeID();
	...
	CUser * pTargetUser = g_pMain->GetUserPtr(pTargetClan->m_strChief, NameType::TYPE_CHARACTER);
	if(pUser->isInGame())
		pUser->SendToRegion(&result,nullptr,pUser->GetEventRoom());
